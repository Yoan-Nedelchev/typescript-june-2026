...
function censorData<T extends { [key: string]: any }>(
  ...
  return function (target: any, key: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = function () {
      const result = original.call(this);
      const censoredResult = censorService.censorProperties(result);
...
function latestOnly(seconds: number) {
  return function (target: any, key: string, descriptor: PropertyDescriptor) {
    const original = descriptor.value;
    ...
    descriptor.value = function () {
      ...
      oldTimestamp.setSeconds(oldTimestamp.getSeconds() - seconds);

      const filteredResult = result.filter(
        (el: { createdOn: Date }) => el.createdOn > oldTimestamp,
      );
...
class UsersService {
  ...
  @log
  @censorData(employeeCensorService)
  @latestOnly(10)
  getEmployees() {
